# 代码审查索引

审查详情见 `codeReviews/` 目录，命名：`YYYY-MM-DD-NN-phase{N}-{scope}-review.md`。

| 日期 | Phase | 审查范围 | 结果 | 文件 |
|------|-------|---------|------|------|
| 2026-07-20 | Phase 0 | 基础层 (3 commits, 25 文件) | 3 BLOCKER + 4 HIGH + 4 MEDIUM | [codeReview/2026-07-20-01-phase0-review.md](codeReviews/2026-07-20-01-phase0-review.md) |
| 2026-07-20 | Phase 1 | 领域逻辑+引擎+连接器 (4 commits, ~20 文件) | 7 BLOCKER + 6 HIGH + 5 MEDIUM | [codeReview/2026-07-20-02-phase1-review.md](codeReviews/2026-07-20-02-phase1-review.md) |
| 2026-07-20 | Phase 2 | Agent 编排 (2 commits) | 3 子代理并行审查 | [codeReview/2026-07-20-03-phase2-review.md](codeReviews/2026-07-20-03-phase2-review.md) |
| 2026-07-20 | Phase 3 | Server 层 (3 commits) | 3 子代理并行审查 | [codeReview/2026-07-20-04-phase3-review.md](codeReviews/2026-07-20-04-phase3-review.md) |
| 2026-07-22 | Phase 4 | Web UI + SSE + 审查修复 | 3 子代理并行审查 | [codeReview/2026-07-22-01-phase4-review.md](codeReviews/2026-07-22-01-phase4-review.md) |
| 2026-07-23 | 全项目 | 全项目综合审查 (166 源文件, 6维度并行) | 15 BLOCKER + 20 HIGH + 20 MEDIUM + 12 LOW | [codeReview/2026-07-23-01-comprehensive-review.md](codeReviews/2026-07-23-01-comprehensive-review.md) |
| 2026-08-02 | Phase 8 基线 | 全量基线审查 (图索引+4子代理) | 8 P0 + 20 P1 + 47 P2 | [codeReview/2026-08-02-01-baseline-review.md](codeReviews/2026-08-02-01-baseline-review.md) |
| 2026-08-02 | Phase 8 A | 批次 A 安全修复 (6 文件) | 初查 1 P0 + 4 P1，已全部修复 | [codeReview/2026-08-02-02-batch-a-security.md](codeReviews/2026-08-02-02-batch-a-security.md) |
| 2026-08-02 | Phase 8 B | 批次 B 编排层修复 (4 文件) | 无 P0/P1；6 P2 中 4 已修，2 留后 | [codeReview/2026-08-02-03-batch-b-orchestration.md](codeReviews/2026-08-02-03-batch-b-orchestration.md) |
| 2026-08-02 | Phase 8 C | 批次 C engine/connector (12 文件) | 无 P0/P1；3 P2 中 3 已修 | [codeReview/2026-08-02-04-batch-c-engine-connector.md](codeReviews/2026-08-02-04-batch-c-engine-connector.md) |
| 2026-08-02 | Phase 8 D | 批次 D llm 模块 (11 文件) | 无 P0/P1；1 P2 留后 | [codeReview/2026-08-02-05-batch-d-llm.md](codeReviews/2026-08-02-05-batch-d-llm.md) |
| 2026-08-02 | Phase 8 E | 批次 E server 层 (5 文件) | 通过；3 P2 中 2 已修 | [codeReview/2026-08-02-06-batch-e-server.md](codeReviews/2026-08-02-06-batch-e-server.md) |
