...
epiphaneia-server
 ...
 │    ├── epiphaneia-engine
 │    │    └── epiphaneia-infra        ← engine 依赖 infra（Connector SPI 类型在此）
 │    ├── epiphaneia-infra
 │    └── epiphaneia-connector         ← agent-core 可通过 infra/api 的 SPI 类型引用 connector 的 Bean
 ├── epiphaneia-infra                  ← infra 不依赖任何内部模块（Connector SPI 自身定义在 infra/api）
 └── epiphaneia-connector              ← connector 依赖 infra（实现 infra/api 的 Connector 接口）

依赖方向：上 → 下。禁止反向。
关键：infra 模块的 pom.xml 不声明对 connector 的编译依赖——Maven 编译期无循环。
      Spring 运行时通过 @Component 扫描注入 connector 的实现 Bean。
```
...
|------|-------------|---------|
| `epiphaneia-connector` | **无 public API** | 纯实现模块——SPI 接口定义在 infra/api。connector 仅含 `@Component` 实现类（PrometheusConnector、ElasticsearchConnector），不对外暴露接口 |
...
| `epiphaneia-infra` | `io.epiphaneia.infra.api` | **Connector SPI**：`Connector<T,R>` 接口、`QueryRequest`、`QueryResult`、`DataSourceType` 常量。**基础设施接口**：`EncryptionService`、`ConnectorRegistry` |
...
**connector ↔ infra 循环依赖修正（RB-1）**：原设计中 `Connector<T,R>` SPI 接口定义在 `connector/api`，`ConnectorRegistry` 在 `infra/api`，形成 `connector → infra（依赖 AuthConfig 等）` 和 `infra → connector（ConnectorRegistry 引用 Connector 类型）` 的双向依赖——Maven 编译期循环。修正方案：将 `Connector<T,R>`、`QueryRequest`、`QueryResult`、`DataSourceType` 从 `connector/api` 上移到 `infra/api`。infra 模块自包含 SPI 定义 + Registry，connector 退化为纯实现模块（仅依赖 infra，实现其 SPI）。Maven 编译期 `infra` 不声明对 `connector` 的依赖——`ConnectorRegistryImpl` 的 `List<Connector>` 类型来自 `infra/api` 自身，Spring 运行时注入 connector 模块的 `@Component` Bean。
...
│   │   └── DiagnosisStateMachine     ← 状态机（CREATED→...→COMPLETED/COMPLETED_PARTIAL/FAILED/ABORTED）
│   ├── llm/
...
```
epiphaneia-connector/                   ← 纯实现模块——SPI 接口在 infra/api
├── io.epiphaneia.connector.internal
│   ├── prometheus/
│   │   └── PrometheusConnector        ← @Component，实现 infra/api/Connector<T,R>
│   └── elasticsearch/
│       └── ElasticsearchConnector     ← @Component，实现 infra/api/Connector<T,R>
```
...
│   ├── connector/                      ← Connector SPI（RB-1 修正：从 connector/api 上移至此）
│   │   ├── Connector                   ← SPI 接口：type(), query(), testConnection()
│   │   ├── QueryRequest / QueryResult ← 泛型基类
│   │   ├── DataSourceType              ← 快捷常量（字符串包装）
│   │   └── AuthConfig                  ← 认证配置值对象
│   ├── EncryptionService               ← 加密服务接口
│   └── ConnectorRegistry               ← Connector 注册与查找。SPI 定义在 infra/api，Registry 自然在 infra
...
新增 Connector 步骤：
1. 实现 `Connector<T,R>` 接口（`io.epiphaneia.infra.api`——SPI 在 infra）
2. 在 `epiphaneia-connector` 模块中添加 `@Component` 注解
3. `type()` 返回新的数据源类型字符串

无需修改 infra 或领域层枚举——ConnectorRegistry 通过 Spring Bean 扫描自动发现新的 Connector。
...
| 简化项 | 影响 | 偿还条件 |
|--------|------|---------|
| ABORTED 终态未实现 | 诊断管道中途可中止（服务重启/超时过期），但编码在 Development 阶段 | Development 阶段实现：状态机新增 ABORTED 终态 + 启动时扫描僵尸诊断并标记 ABORTED |
| 无请求级取消（只跑完不中断） | 用户主动"停止"后 LLM token 浪费 | CANCELLED 状态需求出现时（成本超阈值/用户投诉）。注意 ABORTED（被动中止）≠ CANCELLED（主动取消） |
...
io.epiphaneia
├── connector/
│   └── internal/     ← PrometheusConnector, ESConnector（SPI 在 infra/api）
├── engine/
...
├── infra/
│   ├── api/          ← Connector<T,R> SPI, EncryptionService, ConnectorRegistry
